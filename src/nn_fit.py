...
if rank == 0:
     ...
     plt.axvline(x[argbest], ls='--', color='k', label='Capacity : CV estimate')
...
if rank == 0:
    ...
    plt.axvline(-np.log10(lrates[argbest]), ls='--', color='k', label=r'Learning Rate : CV estimate')
    plt.ylabel(r'$RMSE_{NN}/RMSE_{baseline}$')
...
if rank == 0:
     ...
     plt.axvline(np.log10(bsizes[argbest]), ls='--', color='k', label=r'Batch Size : CV estimate')
     plt.ylabel(r'$RMSE_{NN}/RMSE_{baseline}$')
...
if rank == 0:
     ...
     plt.axvline(-np.log(scales[argbest]), ls='--', color='k', label=r'$\lambda$ : CV estimate')
     plt.ylabel(r'$RMSE_{NN}/RMSE_{baseline}$')
...
if rank == 0:
     ...
     plt.axvline(nepochs[argbest], ls='--', color='k', label=r'Number of Epochs : CV estimate')
     plt.ylabel(r'$RMSE_{NN}/RMSE_{baseline}$')
...
if rank ==0:
    ...
    hp.write_map(oupath+'nn-weights.hp'+str(NSIDE)+'.fits', oumap, fits_IDL=False, overwrite=True, dtype=np.float64)
