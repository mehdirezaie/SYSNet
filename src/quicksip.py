...
def ring_num(nside, z, shift=0):
    ...
    if (z < -2./3.):
       ...
       if (iring == 0):
           iring = 1
       iring = np.long(4*nside - iring)
    # return ring number
    return np.long(iring)
...
def in_ring_simp(nside, iz, phi_low, phi_hi, conservative=True):
    pixmin = np.long(ang2pix_ring_ir(nside,iz,phi_low))
    pixmax = np.long(ang2pix_ring_ir(nside,iz,phi_hi))
    if pixmax < pixmin:
        pixmin1 = pixmax
        pixmax = pixmin
        pixmin = pixmin1
...
def in_ring(nside, iz, phi_low, phi_hi, conservative=True):
    ...
    if (ip_low > ip_hi):
        ...
        to_top = False
    ip_low = np.long( ip_low + ipix1 )
    ip_hi  = np.long( ip_hi  + ipix1 )

    ipix1 = np.long(ipix1)
    if (to_top):
        nir1 = np.long( ipix2 - ip_low + 1 )
        nir2 = np.long( ip_hi - ipix1  + 1 )
        nir  = np.long( nir1 + nir2 )
...
def computeHPXpix_sequ_new(nside, propertyArray, pixoffset=0, ratiores=4, coadd_cut=False): 
    ...
    ipix_list = np.zeros(0, dtype=np.long)
    weight_list = np.zeros(0, dtype=np.float)
